...
function SingleBusCard({bus,handleSelectedBus,withState}) {
  return (
    <div className=' flex items-center flex-row-reverse p-4 gap-4 shadow-sm hover:bg-gray-200 cursor-pointer'onClick={() => handleSelectedBus(bus)} >

    <div className='flex flex-col flex-1'>
        <div className='flex flex-row-reverse items-center gap-1'>
            <div>
              <svg width="17" height="17" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M22 12C22 17.52 17.52 22 12 22C6.48 22 2 17.52 2 12C2 6.48 6.48 2 12 2C17.52 2 22 6.48 22 12Z" stroke="#00DA4C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
              <path d="M15.7109 15.1798L12.6109 13.3298C12.0709 13.0098 11.6309 12.2398 11.6309 11.6098V7.50977" stroke="#00DA4C" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
            </div>
        ...
        <div className='flex flex-row-reverse text-gray-400 items-center gap-1'>
            ...
            <p className='text-xs font-bold text-[#00DA4C]'>
                وقت الوصول
            </p>
    ...
    <div className='flex flex-col flex-1 gap-1'>
        <div className='flex flex-row-reverse items-center  text-right text-sm gap-1'>
            <p >
            {bus.name}
            </p>
            {withState && <p className={`text-[9px] px-1 text-center rounded-md  ${bus.state==='متأخر'?'bg-red-300':'bg-green-300'}`}>{bus.state}</p> }
        </div>
        <div className='flex flex-row-reverse text-gray-400 items-center gap-1'>
             <div>
